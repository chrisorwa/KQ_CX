...
# read data
kq <- read_sheet("https://docs.google.com/spreadsheets/d/1sxNQFslKkGgSt-xbkQzPb2LLfYNRbIRkudG1-abL2Tg/edit?usp=sharing",sheet = "Arrivals")

datetime <- ymd_hms(kq$TIME)
...
delay <- kq %>%
  mutate(
    ...
    TYPE = case_when(
      str_detect(AIRLINE, "Kenya") ~ "Kenya Airways",
      TRUE ~ "Other"
    ),
    FLY = case_when(
      str_detect(AIRLINE, "Cargo") ~ "Cargo",
      TRUE ~ "Passenger"
    )
  ) %>%
